...
1. **Verify Nodes Registered**:
   Check the API to see the connected laptops using a browser:
   `GET http://[SERVER_IP]:8000/get_nodes`
...
3. **Submit a Demo Job**:
   Trigger the demo endpoint to dispatch a training task to the most powerful/trusted available node.
   `POST http://[SERVER_IP]:8000/demo_job`
   *(You can use Swagger UI at `http://[SERVER_IP]:8000/docs` and click "Try it out" on `/demo_job`)*
   
4. **Watch the Real-Time Animated Training**:
   - The selected Node Agent will catch the job, clear the spinner, and load a `tqdm` animated progress bar showing live batches, Accuracy, and Loss drops.
   - The Central Server terminal will stream: `[*] Metrics received for job...`
